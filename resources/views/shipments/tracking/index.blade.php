<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('shipments.show', $shipment) }}" class="text-sm text-blue-600 hover:text-blue-700">
                العودة للشحنة
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-xl p-6 border border-slate-100">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="{{ route('shipments.tracking.create', $shipment) }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700">
                        إضافة سجل تتبع
                    </a>
                </div>

                @if($shipment->trackingRecords->isEmpty())
                ...
                @else
                    <div class="space-y-6">
                        @foreach($shipment->trackingRecords as $index => $record)
                                    ...
                                    <div class="flex items-start justify-between">
                                        ...
                                        <div class="text-left">
                                            ...
                                            <div class="mt-2 flex gap-2">
                                                <a href="{{ route('shipments.tracking.edit', [$shipment, $record]) }}" class="text-xs text-blue-600 hover:text-blue-700">
                                                    تعديل
                                                </a>
                                                <form action="{{ route('shipments.tracking.destroy', [$shipment, $record]) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا السجل؟')">
                                                    @csrf
                                                    @method('DELETE')
            ...
            @if($shipment->currentStage)
                <div class="bg-white shadow-sm sm:rounded-xl p-6 border border-slate-100">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-full flex items-center justify-center" style="background-color: {{ $shipment->currentStage->color ?? '#6366f1' }}; color: white;">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                            </svg>
                        </div>
                        <div>
                            <p class="text-xl font-semibold text-slate-900">{{ $shipment->currentStage->name }}</p>
                            <p class="text-sm text-slate-500">{{ $shipment->currentStage->description }}</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="bg-white shadow-sm sm:rounded-xl p-6 border border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    @foreach($stages as $stage)
                        <div class="p-4 rounded-lg border {{ $shipment->currentStage && $shipment->currentStage->order >= $stage->order ? 'border-green-200 bg-green-50' : 'border-slate-200' }}">
                            <div class="flex flex-col items-center text-center">
                                ...
                                @if($shipment->currentStage && $shipment->currentStage->order >= $stage->order)
                                    <span class="text-xs text-green-600 mt-1">✓ مكتمل</span>
                                @elseif($shipment->currentStage && $shipment->currentStage->order == $stage->order)
