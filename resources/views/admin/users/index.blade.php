<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('admin.users.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                مستخدم جديد
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6 overflow-x-auto">
                <table class="min-w-full text-sm text-right">
                        ...
                        <tr class="border-b last:border-0">
                            ...
                            <td class="py-2">
                                @foreach($user->roles as $role)
                                    <span class="inline-block bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs">{{ $role->name }}</span>
                                @endforeach
                            </td>
                            <td class="py-2 flex items-center gap-2 justify-end">
                                ...
                                <form method="POST" action="{{ route('admin.users.destroy', $user) }}" onsubmit="return confirm('هل تريد حذف المستخدم؟')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-4 text-center text-gray-500">لا يوجد مستخدمون بعد.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
