    ...
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('إدارة حالات الشحنات') }}
            </h2>
            <a href="{{ route('admin.shipment-statuses.create') }}"
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($shipmentStatuses as $status)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-3">
                                            <a href="{{ route('admin.shipment-statuses.edit', $status) }}"
                                            ...
                                            <form method="POST"
                                                action="{{ route('admin.shipment-statuses.destroy', $status) }}"
                                                onsubmit="return confirm('هل أنت متأكد من حذف هذه الحالة؟ لا يمكن التراجع عن هذا الإجراء.')"
                                                class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="2" class="px-6 py-10 text-center text-gray-500">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-base font-medium">لا يوجد حالات شحنات مضافة بعد</p>
                                            <p class="text-sm mt-1">ابدأ بإضافة حالة جديدة لتظهر في هذه القائمة</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
