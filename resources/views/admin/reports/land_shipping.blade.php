    ...
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex gap-2">
                <a href="{{ route('admin.reports.land_shipping.pdf', request()->all()) }}"
                ...
                <form method="GET" action="{{ route('admin.reports.land_shipping') }}"
                    ...
                    <div>
                        ...
                        <x-text-input id="search" class="block w-full" type="text" name="search"
                            :value="request('search')" placeholder="رقم العملية، اسم الشحنة، رقم البيان" />
                    </div>
                    <div>
                ...
                <table class="w-full text-sm text-center text-gray-500 border-collapse border border-gray-300">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-2 py-3 border border-gray-300">المستندات</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($landShipments as $index => $landShipping)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-2 py-2 border border-gray-300">{{ $landShipping->operation_number ?? '-' }}</td>
                                <td class="px-2 py-2 border border-gray-300">{{ $landShipping->shipment_name ?? '-' }}</td>
                                <td class="px-2 py-2 border border-gray-300">{{ $landShipping->declaration_number ?? '-' }}</td>
