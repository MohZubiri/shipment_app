        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-100">
                <div class="p-8 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.shipment-types.update', $shipmentType) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="name" class="block w-full" type="text" name="name"
                                    :value="old('name', $shipmentType->name)" required autofocus placeholder="اسم نوع الشحنة" />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>
                        </div>

                        <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-100 mt-6">
                            <a href="{{ route('admin.shipment-types.index') }}"
