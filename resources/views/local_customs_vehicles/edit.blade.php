<x-app-layout>
            ...
            <form method="POST" action="{{ route('local-shipments.update', $localCustomsVehicle) }}"
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div>
                            ...
                            <select id="department_id" name="department_id"
                                ...
                                @foreach($departments as $department)
                                    <option value="{{ $department->id }}" @selected(old('department_id', $localCustomsVehicle->department_id) == $department->id)>{{ $department->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            <input id="is_active" name="is_active" type="checkbox" value="1"
                                class="rounded border-slate-300 text-blue-600" {{ old('is_active', $localCustomsVehicle->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <div>
                            ...
                            <input id="driver_name" name="driver_name" type="text"
                                value="{{ old('driver_name', $localCustomsVehicle->driver_name) }}"
                                class="mt-2 w-full rounded-md border-slate-300">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700" for="driver_phone">رقم هاتف
                                السائق</label>
                            <input id="driver_phone" name="driver_phone" type="text"
                                value="{{ old('driver_phone', $localCustomsVehicle->driver_phone) }}"
                                class="mt-2 w-full rounded-md border-slate-300">
                        </div>
                        <div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-3">
                        <div>
                            ...
                            <input id="factory_departure_date" name="factory_departure_date" type="date"
                                value="{{ old('factory_departure_date', $localCustomsVehicle->factory_departure_date?->format('Y-m-d')) }}"
                                class="mt-2 w-full rounded-md border-slate-300">
                        </div>
                        <div>
                            ...
                            <select id="warehouse_id" name="warehouse_id"
                                ...
                                @foreach($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}" @selected(old('warehouse_id', $localCustomsVehicle->warehouse_id) == $warehouse->id)>{{ $warehouse->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input id="warehouse_arrival_date" name="warehouse_arrival_date" type="date"
                                value="{{ old('warehouse_arrival_date', $localCustomsVehicle->warehouse_arrival_date?->format('Y-m-d')) }}"
                                class="mt-2 w-full rounded-md border-slate-300">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div id="checkpoints-container" class="space-y-4">
                        @php
                            $checkpoints = old('checkpoints') ?? $localCustomsVehicle->customs;
                        @endphp

                        @foreach($checkpoints as $index => $checkpoint)
                            @php
                                $portId = is_array($checkpoint) ? ($checkpoint['customs_port_id'] ?? '') : $checkpoint->customs_port_id;
                                $entryDate = is_array($checkpoint) ? ($checkpoint['entry_date'] ?? '') : ($checkpoint->entry_date ? $checkpoint->entry_date->format('Y-m-d') : '');
                                $entryTime = is_array($checkpoint) ? ($checkpoint['entry_time'] ?? '') : ($checkpoint->entry_time ? \Carbon\Carbon::parse($checkpoint->entry_time)->format('H:i') : '');
                                $exitDate = is_array($checkpoint) ? ($checkpoint['exit_date'] ?? '') : ($checkpoint->exit_date ? $checkpoint->exit_date->format('Y-m-d') : '');
                                $exitTime = is_array($checkpoint) ? ($checkpoint['exit_time'] ?? '') : ($checkpoint->exit_time ? \Carbon\Carbon::parse($checkpoint->exit_time)->format('H:i') : '');
                            @endphp
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <select name="checkpoints[{{$index}}][customs_port_id]"
                                        class="w-full rounded-md border-slate-300 text-sm" required>
                                        <option value="">اختر الجمرك</option>
                                        @foreach($customsPorts as $port)
                                            <option value="{{ $port->id }}" @selected($portId == $port->id)>{{ $port->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <input type="date" name="checkpoints[{{$index}}][entry_date]" value="{{ $entryDate }}"
                                        class="w-full rounded-md border-slate-300 text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-slate-700 mb-1">وقت الدخول</label>
                                    <input type="time" name="checkpoints[{{$index}}][entry_time]" value="{{ $entryTime }}"
                                        class="w-full rounded-md border-slate-300 text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-slate-700 mb-1">تاريخ الخروج</label>
                                    <input type="date" name="checkpoints[{{$index}}][exit_date]" value="{{ $exitDate }}"
                                        class="w-full rounded-md border-slate-300 text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-slate-700 mb-1">وقت الخروج</label>
                                    <input type="time" name="checkpoints[{{$index}}][exit_time]" value="{{ $exitTime }}"
                                        class="w-full rounded-md border-slate-300 text-sm">
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 shadow-sm">تحديث</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        let checkpointIndex = {{ isset($checkpoints) ? count($checkpoints) : 0 }};
